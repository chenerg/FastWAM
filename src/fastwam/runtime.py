...
def create_fastwam_one(
    model_id: str,
    tokenizer_model_id: str,
    video_action_dit_config,
    tokenizer_max_len: int = 512,
    load_text_encoder: bool = True,
    proprio_dim: int | None = None,
    skip_dit_load_from_pretrain: bool = False,
    ...
    from .models.wan22.fastwam_one import FastWAMOne

    if isinstance(video_action_dit_config, DictConfig):
        video_action_dit_config = OmegaConf.to_container(video_action_dit_config, resolve=True)
    if not isinstance(video_action_dit_config, dict):
        raise ValueError(
            f"`video_action_dit_config` must resolve to a dict, got {type(video_action_dit_config)}"
        )
    ...
    if isinstance(action_scheduler, DictConfig):
        action_scheduler = OmegaConf.to_container(action_scheduler, resolve=True)
    if action_scheduler is None:
        raise ValueError("`action_scheduler` is required for FastWAMOne.")
    if not isinstance(action_scheduler, dict):
        raise ValueError(f"`action_scheduler` must be dict-like, got {type(action_scheduler)}")
    required_action_scheduler_keys = {"train_shift", "infer_shift", "num_train_timesteps"}
    missing_keys = required_action_scheduler_keys - set(action_scheduler.keys())
    ...
    return FastWAMOne.from_wan22_pretrained(
        device=device,
        torch_dtype=model_dtype,
        model_id=model_id,
        tokenizer_model_id=tokenizer_model_id,
        ...
        proprio_dim=(None if proprio_dim is None else int(proprio_dim)),
        redirect_common_files=bool(redirect_common_files),
        video_action_dit_config=video_action_dit_config,
        skip_dit_load_from_pretrain=bool(skip_dit_load_from_pretrain),
        video_train_shift=float(video_scheduler.get("train_shift", 5.0)),
        video_infer_shift=float(video_scheduler.get("infer_shift", 5.0)),
        video_num_train_timesteps=int(video_scheduler.get("num_train_timesteps", 1000)),
        ...
        loss_lambda_action=float(loss.get("lambda_action", 1.0)),
        action_per_frame=(None if action_per_frame is None else int(action_per_frame)),
    )
